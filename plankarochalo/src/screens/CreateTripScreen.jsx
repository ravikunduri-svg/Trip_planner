...
const VIBES = ['Beach', 'Mountains', 'Chill', 'Adventure', 'Party', 'Family-ish']

export default function CreateTripScreen({ onBack, onCreate }) {
  const [name, setName] = useState('')
  const [vibe, setVibe] = useState('')
  const [groupSize, setGroupSize] = useState(6)

  const canCreate = name.trim() && vibe
  ...
  return (
    <div className="max-w-sm mx-auto min-h-dvh flex flex-col bg-white">
      ...
      <div className="flex-1 px-6 py-2 space-y-6">
        ...
        <div>
          <label className="text-sm font-semibold text-gray-700 block mb-2">
            Group size:{' '}
            <span className="text-violet-600">{groupSize} people</span>
          </label>
          <input
            type="range"
            min={2}
        ...
        <div className="bg-violet-50 rounded-2xl p-4 border border-violet-100">
          ...
          <p className="text-xs text-violet-600 mt-1 leading-relaxed">
            You create the trip, share the link, your group votes on everything — destination, dates, budget, stay, activities. You lock decisions. Trip planned.
          </p>
        </div>
      </div>
      ...
      <div className="px-6 pb-10 pt-4">
        <button
          onClick={() => onCreate({ name: name.trim(), vibe, groupSize })}
          disabled={!canCreate}
        ...
        >
          {!name.trim() ? 'Enter a trip name' : !vibe ? 'Pick a vibe' : 'Create Trip 🎯'}
        </button>
      </div>
    </div>
  )
